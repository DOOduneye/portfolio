...
import { metadataFromInput, resourceIdFromInput, resourceTypeFromPath } from "./audit"
...
describe("resourceTypeFromPath", () => {
  it("takes the router segment from an admin path", () => {
    ...
    expect(resourceTypeFromPath("admin.experiences.create")).toBe("experiences")
  })
...
describe("metadataFromInput", () => {
  it("drops the content body so posts are not duplicated into the log", () => {
    expect(metadataFromInput({ slug: "a", title: "A", content: "<p>long</p>" })).toEqual({
      slug: "a",
      title: "A"
    })
  })
