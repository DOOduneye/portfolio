...
export const musicRouter = router({
  topTrack: publicProcedure.query(async ({ ctx }): Promise<TopTrack | null> => {
    const { SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_REFRESH_TOKEN } = ctx.env
    if (!SPOTIFY_CLIENT_ID || !SPOTIFY_CLIENT_SECRET || !SPOTIFY_REFRESH_TOKEN) {
      return null
    }

    const [cached] = await ctx.db.select().from(kvCache).where(eq(kvCache.key, CACHE_KEY))
...
function cachedOrNull(cached: { value: string } | undefined): TopTrack | null {
